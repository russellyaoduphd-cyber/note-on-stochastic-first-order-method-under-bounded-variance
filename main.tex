...
\end{proof}
\section{Prove (3.1) by verifying coefficients match}
Observe that (3.1) is affine in $F$ and $G$. Hence it suffices to verify that the coefficient of each function-value and Gram-matrix term agrees on the two sides. We use the identities from the previous subsection throughout.
\subsection{Constant and Function-Value Terms}
...
\subsection{Gram-Matrix Coefficients}
For brevity, write $s=x_0-x_\star$ and let $b_s$, $b_{g_i}$, and $b_{\epsilon_k}$ denote the corresponding columns of $B_N$. From the definition of $B_N$,
\[b_s=e_1,\qquad b_{g_0}=-T_0e_1+e_2,\qquad b_{g_i}=-e_{i+1}+e_{i+2}\quad(1\le i\le N-1),\]
\[b_{g_N}=-e_{N+1},\qquad b_{\epsilon_k}=-\frac{h_k}{T_{k+1}}e_{k+2}\quad(0\le k\le N-1).\]
Recall that $M_N=K_N^\top\widehat W_NK_N$. Since $Z=B_N^\top M_NB_N$, the coefficient of a diagonal Gram term $\|u\|^2$ in $\langle Z,G\rangle$ is $b_u^\top M_Nb_u$, while the coefficient of an off-diagonal term $\langle u,v\rangle$ is $2b_u^\top M_Nb_v$.
\subsubsection{Terms involving $s$}
The coefficient of $\|s\|^2$ receives contributions only from $\tau\mathcal R(F,G)$ and $\langle Z,G\rangle$. By Lemma~\ref{lem:gram-kernel},
\[-\tau+b_s^\top M_Nb_s=-\frac{1}{2T_0}+\frac{1}{2T_0}=0.\]
Next consider $\langle g_i,s\rangle$. Its interpolation contribution is $\lambda_{\star,i}$, coming from $\lambda_{\star,i}\mathcal Q_{\star,i}$. Using Lemma~\ref{lem:gram-kernel} and the columns of $B_N$ above,
\[2b_s^\top M_Nb_{g_i}=
\begin{cases}
-\alpha_0,&i=0,\\
...
\end{cases}
=-\lambda_{\star,i}.\]
Hence the coefficient of $\langle g_i,s\rangle$ is zero for every $i=0,\ldots,N$.
Finally, the terms involving $\langle\epsilon_k,s\rangle$ come from $\xi_k\mathcal X_{k,0}(F,G)$ and $\langle Z,G\rangle$. By Lemma~\ref{lem:gram-kernel},
\[2b_s^\top M_Nb_{\epsilon_k}=-\frac{2h_k}{T_{k+1}}e_1^\top M_Ne_{k+2}
=-\frac{h_k}{T_{k+1}}\prod_{m=0}^{k}(1-\alpha_m)=-\xi_k.\]
Thus these coefficients also vanish.
\subsubsection{Noise Terms}
For $\|\epsilon_k\|^2$, the only contributions are from $\eta_k\mathcal S_k(F,G)$ and $\langle Z,G\rangle$. Lemma~\ref{lem:gram-kernel} gives
\[b_{\epsilon_k}^\top M_Nb_{\epsilon_k}
=\frac{h_k^2}{T_{k+1}^2}\frac{T_{k+1}}2
=\frac{h_k^2}{2T_{k+1}}=\eta_k,\]
so the total coefficient is $-\eta_k+\eta_k=0$.
For $0\le\ell<k\le N-1$, the terms involving $\langle\epsilon_\ell,\epsilon_k\rangle$ come from $\nu_{\ell,k}\mathcal E_{k,\ell}(F,G)$ and $\langle Z,G\rangle$. Again by Lemma~\ref{lem:gram-kernel},
\[2b_{\epsilon_\ell}^\top M_Nb_{\epsilon_k}
