...
Hendrickx~\cite{rubbens2025}, collect the function-value gaps in
\[F=(f_0-f_\star,\ldots,f_N-f_\star)^\top\in\mathbb R^{N+1}.\]
and the vectors appearing along the trajectory in
\[ V=
\begin{bmatrix}
x_0-x_\star
&
...
\[ x_j-x_\star = x_0-x_\star -\frac{1}{L}\sum_{t=0}^{j-1} h_t(g_t+\epsilon_t), \qquad j=1,\ldots,N. \]
...
After substituting these relations, each
...
Moreover, following the stochastic PEP relaxation, independence and zero mean are encoded through orthogonality constraints with the history information. We write these relations as
...
The identity in~(3.1) is affine in the function values and Gram matrix entries. We therefore verify it by matching the coefficients on the two sides. We first formulate the corresponding PEP relaxation and specify the multipliers, and then verify their feasibility and the claimed identity.
\subsection{Closed form of Multipliers}
...
These formulas completely specify the certificate in \eqref{eq:certificate-identity}. In the next subsection, we verify that the multipliers associated with inequality constraints are nonnegative
and that $Z\succeq0$.
\subsection{Verify Multipliers are non-negative and Prove Positive Semidefiniteness}
\begin{proposition}
...
\end{proof}
It remains to show $Z\succeq0$.
To prove that the Gram multiplier $Z$ is positive semidefinite, we exploit its factorized construction rather than analyze $Z$ entrywise. Since $Z$ is obtained by a congruence transformation of $\widehat W_N$, it suffices to show that $\widehat W_N$ is positive semidefinite. The latter has a simple block-diagonal structure, so the proof reduces to verifying nonnegativity of the scalar blocks $d_j$ and positive semidefiniteness of the leading $2\times2$ block $\Gamma_N$. Both verifications use the short-step condition $h_k\le3/2$.
\begin{proposition}
...
\[ Z=(K_NB_N)^\top\widehat W_N(K_NB_N), \]
it suffices to show that $\widehat W_N\succeq0$. Recall that
\[ \widehat W_N=\Gamma_N\oplus d_2\oplus\cdots\oplus d_N. \]
Thus it remains to show that $d_j\ge0$ for $j=2,\ldots,N$ and that $\Gamma_N\succeq0$.
...
It remains to consider the leading block $\Gamma_N$. Define
\[ \omega=\begin{pmatrix}1\\ T_0(1-\alpha_0)/2\end{pmatrix}. \]
Then
\[ \Gamma_N=\frac{1}{2T_0}\omega\omega^\top+\frac{T_1-T_0(1-\alpha_0)^2}{8}e_2e_2^\top, \]
where $e_2=(0,1)^\top$. The first term is positive semidefinite. For the second coefficient,
\[ T_1-T_0(1-\alpha_0)^2=\alpha_0(2-h_0-\alpha_0). \]
Since $0<\alpha_0\le1/2$ and $h_0\le3/2$, we have
\[ 2-h_0-\alpha_0\ge0. \]
Hence $\Gamma_N\succeq0$. Therefore $\widehat W_N\succeq0$, and consequently
...
\[e_1^\top M_Ne_1=\frac{1}{2T_0},\]
\[e_1^\top M_Ne_{q+2}=\frac12\prod_{m=0}^{q}(1-\alpha_m),
\qquad 0\le q\le N-1,\]
and
\[e_{p+2}^\top M_Ne_{q+2}
=\frac{T_{p+1}}2\prod_{m=p+1}^{q}(1-\alpha_m),\qquad 0\le p\le q\le N-1.\]
\end{lemma}
\begin{proof}
...
\[e_1^\top M_Ne_{q+2} =\frac{1-\alpha_0}{4}\,2\prod_{m=1}^{q}(1-\alpha_m)
...
\[\begin{aligned} e_{p+2}^\top M_Ne_{q+2} =4\prod_{m=p+1}^{q}(1-\alpha_m)\Bigg( &\frac{T_1}{8}\prod_{m=1}^{p}(1-\alpha_m)^2 +\sum_{r=2}^{p+1}d_r\prod_{m=r}^{p}(1-\alpha_m)^2 \Bigg). \end{aligned} \]
...
\end{proof}
\subsection{Prove (3.1) by verifying coefficients match}
Observe that (3.1) is affine in $F$ and $G$. Hence it suffices to verify that the coefficient of each function-value and Gram-matrix term agrees on the two sides. 
\subsubsection{Constant and Function-Value Terms}
The constant terms agree since
\[\tau D^2+\sum_{k=0}^{N-1}\eta_k\sigma^2=\frac{D^2}{2T_0}+\frac{\sigma^2}{2}\sum_{k=0}^{N-1}\frac{h_k^2}{T_{k+1}}=r(h),\]
which follows immediately from the definitions of $\tau$ and $\eta_k$.
Only the interpolation constraints depend on $F$. By Lemmas~\ref{lem:lambda-partial-sum} and~\ref{lem:lambda-outgoing-sum}, for $j=0,\ldots,N-1$, the coefficient of $f_j-f_\star$ is
\[\sum_{i=j+1}^{N}\lambda_{j,i}-\lambda_{\star,j}-\sum_{i=0}^{j-1}\lambda_{i,j}=\alpha_j-\alpha_j=0,\]
where the second $\alpha_j$ follows directly from $\lambda_{\star,0}=\alpha_0$ when $j=0$. For $j=N$, Lemma~\ref{lem:lambda-partial-sum} with $i=N-1$ and $\alpha_N=1$ gives
\[-\lambda_{\star,N}-\sum_{i=0}^{N-1}\lambda_{i,N}=-1.\]
Therefore the function-value terms reduce to $-(f_N-f_\star)$, as required.
\subsubsection{Gram-Matrix Coefficients}
We now match the Gram-matrix terms, beginning with those involving $x_0-x_\star$.
The coefficient of $\|x_0-x_\star\|^2$ receives contributions only from $\tau\mathcal R(F,G)$ and $\langle Z,G\rangle$. The column of $B_N$ corresponding to $x_0-x_\star$ is $e_1$, so Lemma~\ref{lem:gram-kernel} gives
\[-\tau+e_1^\top M_Ne_1=-\frac{1}{2T_0}+\frac{1}{2T_0}=0.\]
Next consider $\langle g_i,x_0-x_\star\rangle$. Its interpolation contribution is $\lambda_{\star,i}$, coming from $\lambda_{\star,i}\mathcal Q_{\star,i}$. The columns of $B_N$ corresponding to $g_i$ are
\[-T_0e_1+e_2\quad(i=0),\qquad -e_{i+1}+e_{i+2}\quad(1\le i\le N-1),\qquad -e_{N+1}\quad(i=N).\]
Since $\langle g_i,x_0-x_\star\rangle$ is an off-diagonal Gram term, applying Lemma~\ref{lem:gram-kernel} gives the contribution from $\langle Z,G\rangle$ as
\[
\begin{cases}
2e_1^\top M_N(-T_0e_1+e_2)=-\alpha_0,&i=0,\\
2e_1^\top M_N(-e_{i+1}+e_{i+2})=-\alpha_i\displaystyle\prod_{m=0}^{i-1}(1-\alpha_m),&1\le i\le N-1,\\
-2e_1^\top M_Ne_{N+1}=-\displaystyle\prod_{m=0}^{N-1}(1-\alpha_m),&i=N,
\end{cases}
\]
which equals $-\lambda_{\star,i}$ in each case. Hence the coefficient of $\langle g_i,x_0-x_\star\rangle$ is zero for every $i=0,\ldots,N$.

The terms involving $\langle\epsilon_k,x_0-x_\star\rangle$ come from $\xi_k\mathcal X_{k,0}(F,G)$ and $\langle Z,G\rangle$. The column of $B_N$ corresponding to $\epsilon_k$ is $-\frac{h_k}{T_{k+1}}e_{k+2}$. Therefore, by Lemma~\ref{lem:gram-kernel}, the contribution from $\langle Z,G\rangle$ is
\[-\frac{2h_k}{T_{k+1}}e_1^\top M_Ne_{k+2}
=-\frac{h_k}{T_{k+1}}\prod_{m=0}^{k}(1-\alpha_m)=-\xi_k,\]
so these coefficients also vanish.

We next consider the noise-noise terms. For $\|\epsilon_k\|^2$, the only contributions are from $\eta_k\mathcal S_k(F,G)$ and $\langle Z,G\rangle$. Using the same column of $B_N$ and Lemma~\ref{lem:gram-kernel}, the contribution from $\langle Z,G\rangle$ is
\[\frac{h_k^2}{T_{k+1}^2}e_{k+2}^\top M_Ne_{k+2}
=\frac{h_k^2}{2T_{k+1}}=\eta_k.\]
Thus the total coefficient is $-\eta_k+\eta_k=0$.
For $0\le\ell<k\le N-1$, the terms involving $\langle\epsilon_\ell,\epsilon_k\rangle$ come from $\nu_{\ell,k}\mathcal E_{k,\ell}(F,G)$ and $\langle Z,G\rangle$. Since this is an off-diagonal Gram term, Lemma~\ref{lem:gram-kernel} gives the contribution from $\langle Z,G\rangle$ as
\[
\frac{2h_\ell h_k}{T_{\ell+1}T_{k+1}}e_{\ell+2}^\top M_Ne_{k+2}
=\frac{h_\ell h_k}{T_{k+1}}\prod_{m=\ell+1}^{k}(1-\alpha_m)
=-\nu_{\ell,k}.
\]
Hence the coefficient of every $\langle\epsilon_\ell,\epsilon_k\rangle$ is zero.

\subsection{Proof of Upper Bound Half of Theorem~\ref{thm:main-rate}}

Two or three sentences that invoke the proposition above.
