...
\end{lemma}
\begin{proof}
Using $\alpha_N=1$ and the definitions of the interpolation multipliers,
\[
\sum_{j=i+1}^{N}\lambda_{i,j}
=\alpha_i\left(\sum_{j=i+1}^{N-1}\alpha_j\prod_{m=i+1}^{j-1}(1-\alpha_m)
+\prod_{m=i+1}^{N-1}(1-\alpha_m)\right).
\]
For each $j=i+1,\ldots,N-1$, factoring out the common product as in the previous lemma gives
\[
\alpha_j\prod_{m=i+1}^{j-1}(1-\alpha_m)
=\prod_{m=i+1}^{j-1}(1-\alpha_m)-\prod_{m=i+1}^{j}(1-\alpha_m).
\]
Therefore the finite sum telescopes: its first positive product is the empty product, hence one, while all intermediate products cancel. Thus
\[
\sum_{j=i+1}^{N-1}\alpha_j\prod_{m=i+1}^{j-1}(1-\alpha_m)
