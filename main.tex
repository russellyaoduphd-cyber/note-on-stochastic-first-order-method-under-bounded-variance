...
\end{proof}
\subsection{Coefficients Match}
Observe that (3.1) is affine in F and G. Hence we can verify the identity by verifying that each associated coefficient matches between the two sides.
...
Next, we match the coefficients of the function-value terms in $F$.
The closed-form multipliers satisfy the telescoping identities
\[\lambda_{\star,j}+\sum_{i=0}^{j-1}\lambda_{i,j}=\alpha_j,\qquad
\sum_{i=j+1}^{N}\lambda_{j,i}=\alpha_j,\qquad j=0,\ldots,N-1,\]
and, using $\alpha_N=1$,
\[\lambda_{\star,N}+\sum_{i=0}^{N-1}\lambda_{i,N}=1.\]
These identities follow directly by telescoping the products in the definitions of the multipliers.
Only the interpolation constraints depend on $F$. Hence, for $j=0,\ldots,N-1$, the coefficient of $f_j-f_\star$ is
\[\sum_{i=j+1}^{N}\lambda_{j,i}-\lambda_{\star,j}-\sum_{i=0}^{j-1}\lambda_{i,j}=0,\]
while the coefficient of $f_N-f_\star$ is $-1$. Therefore the function-value terms reduce to $-(f_N-f_\star)$, as required.
...
\[
e_1^\top M_Ne_{q+2}
=\frac{1-\alpha_0}{4}\,2\prod_{m=1}^{q}(1-\alpha_m)
...
\begin{aligned}
e_{p+2}^\top M_Ne_{q+2}
=4\prod_{m=p+1}^{q}(1-\alpha_m)\Bigg(
&\frac{T_1}{8}\prod_{m=1}^{p}(1-\alpha_m)^2\\
&+\sum_{r=2}^{p+1}d_r\prod_{m=r}^{p}(1-\alpha_m)^2
\Bigg).
\end{aligned}
\]
By the definition
\[
d_r=\frac{T_r-T_{r-1}(1-\alpha_{r-1})^2}{8},
\]
the expression in parentheses becomes
\[
\frac18\left(
T_1\prod_{m=1}^{p}(1-\alpha_m)^2
+\sum_{r=2}^{p+1}
\left[T_r\prod_{m=r}^{p}(1-\alpha_m)^2
-T_{r-1}\prod_{m=r-1}^{p}(1-\alpha_m)^2\right]
\right).
\]
