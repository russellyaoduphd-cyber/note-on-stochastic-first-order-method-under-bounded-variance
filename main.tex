...
\[ \sum_{j=i+1}^{N}\lambda_{i,j} =\alpha_i\left(\sum_{j=i+1}^{N-1}\alpha_j\prod_{m=i+1}^{j-1}(1-\alpha_m) +\prod_{m=i+1}^{N-1}(1-\alpha_m)\right).\]
For each $j=i+1,\ldots,N-1$, 
\[\alpha_j\prod_{m=i+1}^{j-1}(1-\alpha_m)
=\prod_{m=i+1}^{j-1}(1-\alpha_m)-\prod_{m=i+1}^{j}(1-\alpha_m).\]
Recall that the empty product is defined to be $1$ and use the telescoping sum, \[ \sum_{j=i+1}^{N-1}\alpha_j\prod_{m=i+1}^{j-1}(1-\alpha_m) =1 \prod_{m=i+1}^{N-1}(1-\alpha_m). \]
Substituting this above gives
\[ \sum_{j=i+1}^{N}\lambda_{i,j}=\alpha_i. \]
\end{proof}
