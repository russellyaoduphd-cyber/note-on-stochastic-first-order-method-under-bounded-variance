...
\section{Section 1 Reformulation}
...
Taking expectations, define the interpolation quantities
\[ \mathcal Q_{i,j}(F,G) := \mathbb E_\epsilon\!\left[ f_i-f_j -\langle g_j,x_i-x_j\rangle -\frac{1}{2L}\|g_i-g_j\|^2 \right], \qquad i,j\in\mathcal I_N . \]
To express these quantities using finitely many variables, following the
stochastic PEP framework of Rubbens, Colla, and
Hendrickx~\cite{rubbens2025}, collect the expected function values in
\[ F= \left( f_\star, \mathbb E_\epsilon[f_0], \ldots, \mathbb E_\epsilon[f_N] \right)^\top \]
...
\epsilon_{N-1}
\end{bmatrix}. \]
We define the expected Gram matrix
...
and \[ \mathcal S_k(F,G) := \sigma^2-\mathbb E_\epsilon\|\epsilon_k\|^2 \geq0, \qquad k=0,\ldots,N-1. \]
Moreover, independence and zero mean imply that each noise vector is
orthogonal in expectation to all information available before it is
sampled. We encode these relations by
\[ \mathcal E_{k,\ell}(F,G) := \mathbb E_\epsilon \langle\epsilon_k,\epsilon_\ell\rangle =0, \qquad 0\leq\ell<k\leq N-1, \]
\[ \mathcal G_{k,j}(F,G) := \mathbb E_\epsilon \langle\epsilon_k,g_j\rangle =0, \qquad 0\leq j\leq k\leq N-1, \]
and
\[ \mathcal X_{k,0}(F,G) := \mathbb E_\epsilon
\langle\epsilon_k,x_0-x_\star\rangle =0, \qquad k=0,\ldots,N-1. \]
Together with the SGD recursion, these equalities also imply
$\mathbb E_\epsilon\langle\epsilon_k,x_j-x_\star\rangle=0$
for every $j\leq k$.
...
\end{remark}
Our proof constructs explicit multipliers, given in closed form below, such that \begin{equation}\label{eq:certificate-identity}
\begin{aligned}
...
\subsection{Closed-Form Multipliers}
For the remainder of the proof, we normalize $L=1$; the general result follows by
rescaling the objective and stochastic gradients. We first consider
$h\in(0,1.5]^N$, with the boundary cases obtained by continuity.
Recall\[R_k=\sum_{t=k}^{N-1}h_t,\qquad R_N=0,\]
and define
\[
T_k:=1+2R_k,
\qquad
\alpha_k:=\frac{h_k}{2R_k},
\qquad
k=0,\ldots,N-1.
\]
...
Let
\[
\mathcal I_N=\{\star,0,\ldots,N\},
\qquad
g_\star=0,
\qquad
s:=x_0-x_\star.
\]
We collect the vectors relevant to the trajectory in
\[
V=
\begin{bmatrix}
s & g_0 & \cdots & g_N &
\epsilon_0 & \cdots & \epsilon_{N-1}
\end{bmatrix}
\]
...
G:=V^\top V.
\]
We also collect the function values in
\[
F=(f_\star,f_0,\ldots,f_N)^\top.
\]

For every $i,j\in\mathcal I_N$, define the smooth convex interpolation
quantity
\[
\mathcal Q_{i,j}(F,G)
:=
f_i-f_j
-\langle g_j,x_i-x_j\rangle
-\frac12\|g_i-g_j\|^2.
\]
The remaining inequality constraints are
\[
\mathcal R(F,G)
:=
D^2-\|x_0-x_\star\|^2
...
k=0,\ldots,N-1.
\]
The orthogonality conditions are represented by the affine equalities
\[
\mathcal E_{k,\ell}(F,G)
:=
\langle\epsilon_k,\epsilon_\ell\rangle,
\qquad
0\leq \ell<k\leq N-1,
\]
\[
...
\mathcal X_{k,0}(F,G)
:=
\langle\epsilon_k,x_0-x_\star\rangle,
\qquad
k=0,\ldots,N-1.
\]
Indeed, each $x_j-x_\star$ is a linear combination of
$x_0-x_\star$, previous gradients, and previous noise terms, so no
additional iterate vectors are needed in the Gram representation.

Using these quantities, we consider the Gram relaxation
\begin{equation}
\label{eq:gram-pep}
p_{\mathrm{gram}}(h)
=
...
=
-
\frac{h_\ell h_k}{T_{k+1}}
\prod_{m=\ell+1}^{k}(1-\alpha_m),
\qquad
0\leq\ell<k\leq N-1.
\]
...
\widehat W_N
=
\Gamma_N\oplus d_2\oplus\cdots\oplus d_N,
\]
where
\[
\Gamma_N
...
These formulas completely specify the certificate in
\eqref{eq:certificate-identity}. In the next subsection, we verify that
the multipliers associated with inequality constraints are nonnegative
and that $Z\succeq0$.
