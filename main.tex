...
For any $h\in(0,3/2]^N$, the multipliers defined above satisfy the certificate identity~\eqref{eq:certificate-identity} for every $F\in\mathbb R^{N+1}$ and every symmetric matrix $G\in\mathbb R^{(2N+2)\times(2N+2)}$.
\end{proposition}
...
\subsubsection{terms involving initial distance}
...
\[-\frac{2h_k}{T_{k+1}}e_1^\top M_Ne_{k+2} =-\frac{h_k}{T_{k+1}}\prod_{m=0}^{k}(1-\alpha_m) =-\xi_k.\]
...
\[ \frac{h_k^2}{T_{k+1}^2}e_{k+2}^\top M_Ne_{k+2} =\frac{h_k^2}{2T_{k+1}}
=\eta_k. \]
